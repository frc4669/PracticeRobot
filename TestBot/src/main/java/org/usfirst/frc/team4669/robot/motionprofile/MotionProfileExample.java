...
// * Although this code uses a CANTalon, nowhere in this module do we
// changeMode() or call set() to change the output.
// * This is done in Robot.java to demonstrate how to change control modes on
// the fly.
// *
...
// * clearMotionProfileHasUnderrun to get status and potentially clear the error
// flag.
// *
// * pushMotionProfileTrajectory
// * clearMotionProfileTrajectories
// * processMotionProfileBuffer, to push/clear, and process the trajectory
// points.
// *
// * getControlMode, to check if we are in Motion Profile Control mode.
// *
// * Example of advanced features not demonstrated here...
// * [1] Calling pushMotionProfileTrajectory() continuously while the Talon
// executes the motion profile, thereby keeping it going indefinitely.
// * [2] Instead of setting the sensor position to zero at the start of each MP,
// the program could offset the MP's position based on current position.
// */
...
// * call set(), just get motion profile status and make decisions based on
// motion
// * profile.
// */
...
// good
// * idea to add a timeout. Set to -1 to disable. Set to nonzero to count down
// to
// * '0' which will print an error message. Counting loops is not a very
// accurate
...
// * It doesn't need to be very accurate, just needs to keep pace with the
// motion
// * profiler executer. Now if you're trajectory points are slow, there is no
// need
...
// _talonR = talonR;
// /*
// * since our MP is 10ms per point, set the control frame rate and the notifer
// to
// * half that
// */
...
// public void reset() {
// /*
// * Let's clear the buffer just in case user decided to disable in the middle
// of
...
// if (_statusL.btmBufferCnt > kMinPointsInTalon && _statusR.btmBufferCnt >
// kMinPointsInTalon) {
...
// * this is so that you can unplug your talon in the middle of an MP and react
// to
// * it.
// */
...
// }
// /*
// * If we are executing an MP and the MP finished, start loading another. We
// will
// * go into hold state so robot servo's position.
// */
// if (_statusL.activePointValid && _statusL.isLast && _statusR.activePointValid
// && _statusR.isLast) {
// /*
// * because we set the last point's isLast to true, we will get here when the
// MP
// * is done
// */
...
// Instrumentation.process(_statusL, _posL, _velL, _headingL, _statusR, _posR,
// _velR, _headingR);
// }
// }
...
// "Trajectory Duration not supported - use configMotionProfileTrajectoryPeriod
// instead", false);
// }
// /* pass to caller */
// return retval;
...
// startFilling(_path.getLeft(), _path.getRight(), _path.kNumPointsL(),
// _path.kNumPointsR());

// }

// private void startFilling(double[][] profileL, double[][] profileR, int
// totalCntL, int totalCntR) {
...
// * below
// */
// _talonL.configMotionProfileTrajectoryPeriod(Constants.baseTrajPeriodMs,
// Constants.timeout);
// _talonR.configMotionProfileTrajectoryPeriod(Constants.baseTrajPeriodMs,
// Constants.timeout);
...
// pointL.position = positionRotL * Constants.encoderTicksPerRotation; //
// Convert Revolutions to Units
// pointL.velocity = velocityRPML * Constants.encoderTicksPerRotation / 600.0;
// // Convert RPM to Units/100ms
// pointL.headingDeg = 0; /* future feature - not used in this example */
// pointL.profileSlotSelect0 = 0; /* which set of gains would you like to use
// [0,3]? */
// pointL.profileSlotSelect1 = 0; /*
...
// pointR.position = positionRotR * Constants.encoderTicksPerRotation; //
// Convert Revolutions to Units
// pointR.velocity = velocityRPMR * Constants.encoderTicksPerRotation / 600.0;
// // Convert RPM to Units/100ms
// pointR.headingDeg = 0; /* future feature - not used in this example */
// pointR.profileSlotSelect0 = 0; /* which set of gains would you like to use
// [0,3]? */
// pointR.profileSlotSelect1 = 0; /*
// * future feature - not used in this example - cascaded PID [0,1], leave
// * zero
// */
